...
## Qual textura usar como base pro cálculo do glow (blur/detecção).
## Só funciona corretamente se under/progress/over forem a MESMA imagem.
enum GlowSourceTexture { UNDER, PROGRESS, OVER }
@export var glow_source_texture: GlowSourceTexture = GlowSourceTexture.PROGRESS:
	set(value):
		glow_source_texture = value
		_assign_glow_source()
...
func _assign_glow_source():
	if not _ensure_material():
		return
	var tex: Texture2D
	match glow_source_texture:
		GlowSourceTexture.UNDER:
			tex = texture_under
		GlowSourceTexture.PROGRESS:
			tex = texture_progress
		GlowSourceTexture.OVER:
			tex = texture_over
	if tex != null:
